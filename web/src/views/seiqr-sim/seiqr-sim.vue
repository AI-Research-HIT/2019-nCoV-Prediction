<template>
  <section class="chart-container">
    ...
    <el-row>
      ...
      <el-col :span="8">
        <div class="chinaMap" id="chinaMapBefore"></div>
      </el-col>
      <el-col :span="8">
        <div class="chinaMap" id="chinaMapNow"></div>
      </el-col>
      <el-col :span="8">
        <div class="chinaMap" id="chinaMapAfter"></div>
      </el-col>
    </el-row>
    <H3>拖动选择上图对比日期</H3>
...
export default {
  ...
  methods: {
    ...
    sliderChange() {
      ...
      if (this.modelResult.length > 0) {
        if (
          ...
          this.slidervalue[0] < this.modelResult.length
        ) {
          var before = {
            name: this.selectedCity,
            value: this.modelResult[this.slidervalue[0]]["predictTotal"]
          };
          //console.log(before)
          this.beforeMapOption.series[0].data = [];
          this.beforeMapOption.series[0].data.push(before);
          this.beforeMapOption.title.text =
            this.modelResult[this.slidervalue[0]]["date"] +
            "：预测确诊" +
            before.value;
        ...
        if (
          ...
          this.slidervalue[1] < this.modelResult.length
        ) {
          var after = {
            name: this.selectedCity,
            value: this.modelResult[this.slidervalue[1]]["predictTotal"]
          };
          //console.log(after)
          this.afterMapOption.title.text =
            this.modelResult[this.slidervalue[1]]["date"] +
            "：预测确诊" +
            after.value;
    ...
    calMapDate(data, idx) {
      ...
      this.nowMapOption.title.text =
        data[idx]["date"] + "：预测确诊" + now.value;
      chinaChartNow.setOption(this.nowMapOption);

      var after = {
        name: this.selectedCity,
        value: data[data.length - 1]["predictTotal"]
      };
      this.afterMapOption.title.text =
        data[data.length - 1]["date"] + "：预测确诊" + after.value;
      this.afterMapOption.series[0].data = [];
      this.afterMapOption.series[0].data.push(after);
      chinaChartAfter.setOption(this.afterMapOption);
    },
    drawMap() {
      ...
      chinaChartNow = echarts.init(document.getElementById("chinaMapNow"));
      chinaChartAfter = echarts.init(document.getElementById("chinaMapAfter"));
      chinaChartBefore.setOption(this.beforeMapOption);
      chinaChartNow.setOption(this.nowMapOption);
      chinaChartAfter.setOption(this.afterMapOption);
    },
